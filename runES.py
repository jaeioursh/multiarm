...
def post_train(fidx):
    writer = SummaryWriter("./logs/es"+str(fidx))
    fname = "logs/BASE.dat"
    with open(fname, "rb") as f:
        learner = pkl.load(f)
        learner.agent.LR = 0.001
        learner.agent.sigma = 0.003
        learner.agent.hof_freq = 10
        learner.agent.best_r = -1e9
    with mp.Pool(12) as pool:
        for gen in range(10000):
...
def pretrain():
    ...
    params.shape = [[env.state_dim, 128, 64],
                    env.n_agents, [64, env.action_dim]]
    params.state_dim = env.state_dim
    learner = MHES(params)
    for gen in range(2000):
        ...
        for idx in range(learner.popsize):
            ...
            diff = -np.mean(diff)
            r.append(diff)
        print(gen, max(r), learner.agent.sigma)
        learner.train(r)
